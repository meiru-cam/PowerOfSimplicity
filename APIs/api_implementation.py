...
# Calculate data directory path relative to this file (works from any directory)
_api_dir = os.path.dirname(os.path.abspath(__file__))
_mirai_root = os.path.dirname(_api_dir)
_data_dir = os.path.join(_mirai_root, 'data', 'MIRAI')

# load kg data
data_kg = pd.read_csv(os.path.join(_data_dir, 'data_kg.csv'), sep='\t', dtype=str)
...
# load news data
data_news = pd.read_csv(os.path.join(_data_dir, 'data_news.csv'), sep='\t', dtype=str)
# columns: Docid, MD5, URL, Date, Title, Text, Abstract

# load country data
_info_dir = os.path.join(_mirai_root, 'data', 'info')
dict_iso2alternames = json.load(open(os.path.join(_info_dir, 'dict_iso2alternames_GeoNames.json')))
country_embeddings = np.load(os.path.join(_info_dir, 'country_embeddings.npy'))
...
# load relation data
dict_code2relation = json.load(open(os.path.join(_info_dir, 'dict_code2relation.json')))
relation_embeddings = np.load(os.path.join(_info_dir, 'relation_embeddings.npy'))
...
def get_relation_distribution(date_range: Optional[DateRange] = None, head_entities: Optional[List[ISOCode]] = None, tail_entities: Optional[List[ISOCode]] = None) -> Dict[CAMEOCode, int]:
    ...
    # sort the dictionary by values in descending order
    relation_counts = dict(sorted(relation_counts.items(), key=lambda item: item[1], reverse=True))
    if os.environ["TOP_10_REL"] == "True":
        # Keep only the top 10 items
        relation_counts = dict(list(relation_counts.items())[:10])
    # Convert keys to CAMEOCode objects (if needed)
    relation_counts = {CAMEOCode(key): value for key, value in relation_counts.items()}
