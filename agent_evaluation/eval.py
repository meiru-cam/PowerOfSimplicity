...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name", type=str, default="gpt-3.5-turbo-0125",
                        choices=["gpt-3.5-turbo-0125", # latest GPT-3.5 turbo model (Sep 2021)
                                 ...
                                 "gpt-4o-mini", # gpt-4o-mini point to gpt-4o-mini-2024-07-18
                                 "Meta-Llama-3-8B-Instruct", # Meta-Llama 3 model (March 2023)
                                 "Llama-3.1-8B-Instruct", # Meta-Llama 3.1 model (July 23, 2024)
                                 "Llama-3.2-3B-Instruct", # Meta-Llama 3.1 model 
                                 "Llama-3.1-70B-Instruct", # Meta-Llama 3.1 model
    ...
    parser.add_argument("--plan", type=str, default="react", help="planning strategy") # choices=["direct", "cot", "react"],
    parser.add_argument("--action", type=str, default="func", choices=["func", "block", "none"], help="action type")
    parser.add_argument("--api", type=str, default="full", choices=["full", "kg", "news", "none", "partial"], help="api type")
    ...
    parser.add_argument("--news_k", type=int, default=15, help="total news article limit", required=False)

    parser.add_argument("--exact_output_dir", type=str, default="", help="exact output directory")
    parser.add_argument("--exact_output_eval_dir", type=str, default="", help="exact output evaluation directory")
    ...
    # set environment variables for diversity and event_k
    event_diversity = args.event_diversity
    event_k = args.event_k
    os.environ['EVENT_DIVERSITY'] = str(event_diversity)
    os.environ['TOTAL_EVENT_LIMIT'] = str(event_k)
    article_diversity = args.article_diversity

    if args.exact_output_dir != "":
        setting_output_dir = args.exact_output_dir
    else:
        if not os.path.exists(args.output_dir):
            os.makedirs(args.output_dir)
        
        if "simplerag" in args.output_dir:
            setting_output_dir = os.path.join(args.output_dir, args.dataset, args.model_name, "timediff{}-{}-eventK{}-div{}-temp{}".format(args.timediff, args.plan, args.event_k, args.event_diversity, args.temperature))
        else:
        ...
        # setting_output_dir += "-event_diversity{}".format(event_diversity)
        # setting_output_dir += "-eventk{}".format(event_k)
    ...
    # TODO: Why is this here in the original code?
    ...
    if args.exact_output_eval_dir != "":
        eval_save_path = args.exact_output_eval_dir
    else:
        ...
        # eval_file += "-event_diversity{}".format(event_diversity)
        # eval_file += "-eventk{}".format(event_k)
        # eval_file += "-article_diversity{}".format(args.article_diversity)

        if "simplerag" in args.output_dir:
            eval_file = "round{}-{}-timediff{}-{}-eventK{}-div{}-temp{}".format(args.rounds, args.model_name, args.timediff, args.plan, args.event_k, args.event_diversity, args.temperature)

        if args.alias != "":
            eval_file = eval_file + '-' + args.alias
        eval_save_path = os.path.join(eval_dir, eval_file)
    json.dump(eval_results, open(os.path.join(eval_save_path + '.json'), 'w'), indent=4)
    eval_df.to_csv(os.path.join(eval_save_path + '.csv'), index=False, sep='\t')