...
# from agent_prompts.prompt_extraction_extract_only import extraction_prompt
...
class DirectAgent:
    ...
    def _build_agent_prompt(self) -> str:
        ...
        return self.agent_prompt.format(
            ...
            actor1_code = self.query_info['Actor1CountryCode'],
            actor2_code = self.query_info['Actor2CountryCode'],
            events = self.query_info['Events'],
            article_titles = self.query_info['NewsArticles'],
            # direct_events = self.query_info['DirectEvents'],
            # third_party_events = self.query_info['ThirdPartyEvents'],
            cameo_description = self.query_info['CameoDescription'],
            relation_distribution = self.query_info['RelationDistribution']
            )
...
if __name__ == "__main__":
    ...
    # Unified: always use the default extraction prompt for SimpleRAG
    print("Using simpleRAG_default extraction prompt")
    from agent_prompts.prompt_extraction_simpleRAG_default import extraction_prompt
    ...
    setting_output_dir = os.path.join(args.output_dir, args.dataset, args.model_name, "timediff{}-{}-eventK{}-div{}-temp{}".format(args.timediff, args.plan, args.event_k, args.diversity, args.temperature))
    ...
    # Strip "simplerag_" prefix if present
    plan_key = args.plan.replace("simplerag_", "")
    
    PROMPT_MODULE_MAP = {
        "e_bi": "prompts_simplerag_events",
        "e_uni": "prompts_simplerag_events",
        "art_bi_title": "prompts_simplerag_articles",
        "art_uni_title": "prompts_simplerag_articles",
        "rel_e_bi": "prompts_simplerag_rel_events",
        "rel_e_uni": "prompts_simplerag_rel_events",
        "rel": "prompts_simplerag_rel_events",
        "rel_e_bi_art_bi_title": "prompts_simplerag_rel_events_articles",
        "rel_e_uni_art_uni_title": "prompts_simplerag_rel_events_articles",
        # Add more mappings as needed
    }
    prompt_module_name = PROMPT_MODULE_MAP.get(plan_key, f'prompts_{args.plan}')
        ...
        for curr_round in range(args.rounds):
            ...
            for rowid, row in tqdm(data_query.iterrows(), total=len(data_query)):
                ...
                if args.event_k == 0:
                    ...
                    # direct_events, third_party_events, events = get_events(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode'])])
                    # direct_events, third_party_events, events = get_events_temporal_diverfied(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode'])])
                    
                    query_text = f'Please forecast the relations that {row["Actor1CountryName"]} will take towards {row["Actor2CountryName"]} on {future_date_nlp} based on your knowledge up to {curr_date_nlp}.'
                    # direct_events, third_party_events, events = get_events_novelty_diversified(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode'])], text_description=query_text)
                    if "e_bi" in args.plan:
                        if "bm25" in args.alias:
                            events = get_events(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode']), ISOCode(row['Actor2CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode']), ISOCode(row['Actor1CountryCode'])], text_description=query_text)
                        else:
                            events = get_events(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode']), ISOCode(row['Actor2CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode']), ISOCode(row['Actor1CountryCode'])]) #, text_description=query_text)
                    else:
                        #"e_uni" in args.plan:
                        if "bm25" in args.alias:
                            events = get_events(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode'])], text_description=query_text)
                        else:
                            events = get_events(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode'])]) #, text_description=query_text)
                    ...
                    # add relation distribution to the row
                    relation_distribution = get_relation_distribution(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode'])])

                    # if "rel10" in args.plan:
                    #     top_10_items = heapq.nlargest(10, relation_distribution.items(), key=lambda item: item[1])
                    #     relation_distribution = dict(top_10_items)
                    
                    # print(dict(relation_distribution))
                    codes_in_rel_dist = [item[0] for item in relation_distribution.items()]
                    row['RelationDistribution'] = relation_distribution
                    
                    # Get CAMEO code
                    list_relation_codes = [event.relation for event in events]
                    # add code from relation distribution
                    list_relation_codes.extend(codes_in_rel_dist)


                    # get the unique relation codes
                    list_relation_codes = list(set(list_relation_codes))
                    print(list_relation_codes)
                    list_relation_codes = sorted(list_relation_codes, key=lambda x: int(x.code))
                    # save the subset of relations that appeared in the events
                    cameo_descriptions = {relation: map_cameo_to_relation(relation) for relation in list_relation_codes}
                    row['CameoDescription'] = cameo_descriptions

                if "art_bi" in args.plan:
                    art_titles = get_news_articles(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode']), ISOCode(row['Actor2CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode']), ISOCode(row['Actor1CountryCode'])])
                else:
                    art_titles = get_news_articles(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode'])])
                
                row['NewsArticles'] = art_titles

                # if args.news_k == 0:
                #     row["NewsArticles"] = []
                # else:
                #     # retrieve news articles using get_news_articles
                #     news_articles = get_news_articles(date_range=DateRange(start_date=Date("2023-01-01"), end_date=Date(curr_date_str)), head_entities=[ISOCode(row['Actor1CountryCode'])], tail_entities=[ISOCode(row['Actor2CountryCode'])])
                #     row["NewsArticles"] = news_articles
    ...
    eval_dir = os.path.join(f"../output_eval/{args.output_dir.split('/')[-1]}", args.dataset)
    ...
    eval_file = "round{}-{}-timediff{}-{}-eventK{}-div{}-temp{}".format(args.rounds, args.model_name, args.timediff, args.plan, args.event_k, args.diversity, args.temperature)
    if args.alias != "":
